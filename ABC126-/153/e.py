...
def main():
    ...
    for i in range(n):
        x, h = xh[i]
        bit = 0 if h % a == 0 else 1
        xs.append(x)
        counts.append((h // a) + bit)

    right = [-1]*n
    r = 0
    for l in range(n):
        right[l] = bisect_right(xs, xs[l]+2*d)
    ...
    #以下O(n**2)なので高速化したい -> lazy segment tree で高速化したO(nlogn)
    # -> 解説は累積和で高速化
    ans = 0
    lst = LazySegTree(n, op, e, composition, id_, mapping)
    lst.initialize(counts)
    for i in range(n):
        count = max(0, lst.get(i))
        ans += count
        lst.range_apply(i, right[i], count)
    print(ans)
