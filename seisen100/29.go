...
func main() {
	...
	field := make([][]string, R)
	for i := range field {
		field[i] = strings.Split(scanString(), "")
	}
	q := NewQueue()
	visited := make([]int, R*C)
	for i := range visited {
		visited[i] = -1
	}
	start := convertToNum(sy, sx, C)
	visited[start] = 0
	q.Push(start)
	D := [][2]int{
		{0, 1},
	...
	for !q.IsEmpty() {
		pNum := q.Pop()
		py, px := convertToPoint(pNum, C)
		for _, d := range D {
			cy := py + d[0]
			cx := px + d[1]
			cNum := convertToNum(cy, cx, C)
			if field[cy][cx] == "#" {
				continue
			}
			if visited[cNum] != -1 {
			...
			visited[cNum] = visited[pNum] + 1
			q.Push(cNum)
		}
	}
	gNum := convertToNum(gy, gx, C)
	out(visited[gNum])
}
func convertToNum(y, x, C int) int {
	return y * C + x
}
func convertToPoint(v , C int) (int, int) {
	x := v % C
	y := v / C
	return y, x
}
