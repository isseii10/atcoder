...
func main() {
	...
	field := make([][]int, R)
	for i := range field {
		field[i] = scanIntSlice(C)
	}
	ansList := make([][2]int, C)
	for i := 0; i<R; i++ {
		for j:=0;j<C;j++ {
			if field[i][j] == 1 {
				ansList[j][1]++
	...
	ans := 0
	for i := 0; i < 1<<R; i++ {
		copyAnsList := make([][2]int, C)
		_ = copy(copyAnsList, ansList)
		for j:=0; j<R;j++ {
			if i>>j&1 == 1 {
				for k := range field[j] {
					if field[j][k] == 1 {
		...
		tmp := 0
		for c := range copyAnsList {
			tmp += max(copyAnsList[c][0], copyAnsList[c][1])
		}
		ans = max(ans, tmp)
	}
	out(ans)
