...
func main() {
	...
	E, F := scanInt2()
	X, Y := scanIntSlice2(m)
	mazeMap := make(map[[2]int]bool)
	for i := 0; i < m; i++ {
		X[i]--
		Y[i]--
		if _, ok := mazeMap[[2]int{X[i], Y[i]}]; !ok {
			mazeMap[[2]int{X[i], Y[i]}] = false
		}
		mazeMap[[2]int{X[i], Y[i]}] = true
	}
	dp := make([][][]int, n+1)
	for i := range dp {
		dp[i] = make([][]int, n+1)
		...
		for j := 0; j <= n; j++ {
			for k := 0; k <= n; k++ {
				if i + j + k == n {
					nowX := i*A + j*C + k*E 
					nowY := i*B + j*D + k*F 
					if _, ok := mazeMap[[2]int{nowX, nowY}]; ok {
						dp[i][j][k] = 0
						continue
					}
				}
