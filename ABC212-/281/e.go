...
func main() {
	...
	avl := NewAVLTree()
	for i:=0;i<M;i++ {
		avl.Insert(Key(A[i]), Value(A[i]))	
	}
	for { 
		if key, ok := avl.FindKthElement(K); ok {
			out(fmt.Sprintf("%vth key is %v", K, key))	
			avl.Delete(key)
		} else {
			break
		}
	...
	ans := 0
	for i := 0; i<K;i++ {
		key, ok := avl.FindKthElement(i)
		if ok {
			v, _ := avl.Get(key)
			ans += int(v)
			out(fmt.Sprintf("%vth key is %v, value is %v", i, key, v))	
		}
	}
	out(ans)
	...
	for i := M;i<N;i++ {
		// 抜けるkeyを消す
		delKey := A[i-M]
		if ok := avl.Delete(Key(delKey)); ok {
			ans -= delKey	
		}
		// 追加のkeyを足す
		inKey := A[i]
		ans += inKey
		avl.Insert(Key(inKey), Value(inKey))	
		// K番目にあぶれたものは消す
		key, _ := avl.FindKthElement(K)
		ans -= int(key)
		for i := 0; i<K;i++ {
			key, ok := avl.FindKthElement(i)
			if ok {
