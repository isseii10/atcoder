...
func main() {
	...
	if len(sCounter) != len(tCounter) {
		...
		for i:=0;i<len(sCounter);i++ {
			if sCounter[i].str != tCounter[i].str {
				isAble = false
				break
			} else {
				if sCounter[i].num > tCounter[i].num {
					isAble = false
					break
				}
				if sCounter[i].num < tCounter[i].num && sCounter[i].num == 1 {
					isAble = false
					break
				}
			}
...
func uni(s []string) []Counter {
	ret := make([]Counter, 0)
	c := Counter{str:s[0], num:1}
	for i:=1;i<len(s);i++ {
		if s[i] == c.str {
			c.num++
		} else {
			ret = append(ret, c)
			c = Counter{str:s[i], num:1}
		}
	}
	ret = append(ret, c)
	return ret
