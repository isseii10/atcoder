...
func main() {
	defer flush()
	n := scanInt()
	sx, sy, tx, ty := scanInt4()
	Cs := make([]circle, n)
	for i:=0;i<n;i++ {
		x, y, r := scanInt3()
		Cs[i] = circle{x: x, y: y, r: r}
	}
	start := findCircle(sx, sy, Cs)
	goal := findCircle(tx, ty, Cs)
	graph := newGraph(n)
	for i :=0;i<n;i++ {
		for j:=0;j<n;j++ {
			if i==j {continue}
			if isCross(Cs[i], Cs[j]) {
				graph[i] = append(graph[i], edge{to: j})
				graph[j] = append(graph[j], edge{to: i})
			}
		}
	}
...
func isCross(c1, c2 circle) bool {
	dist :=(c1.x - c2.x)*(c1.x - c2.x)+(c1.y - c2.y)*(c1.y - c2.y)
	if dist <= (c1.r+c2.r)*(c1.r+c2.r)  && dist >= (c1.r-c2.r)*(c1.r-c2.r){
		return true
	}
	return false
}
func findCircle(x, y int, Cs []circle) int {
	for i, c := range Cs {
