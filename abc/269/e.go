...
func query(a, b, c, d int) int {
	q := fmt.Sprintf("? %v %v %v %v", a, b, c, d)
	fmt.Println(q)
	var ret int
	fmt.Scan(&ret)
	if ret == -1 {
...
func bisectAB(n, a, b int) int {
	for a != b {
		mid := (a+b) / 2
		if query(a, mid, 1, n) == mid - a + 1 {
			a = mid + 1
		} else {
			b = mid
		}
...
func bisectCD(n, a, b int) int {
	for a != b {
		mid := (a+b) / 2
		if query(1, n, a, mid) == mid - a + 1 {
			a = mid + 1
		} else {
			b = mid
		}
