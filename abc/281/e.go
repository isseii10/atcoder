...
func main() {
	...
	N, M, K := scanInt3()
	A := scanIntSlice(N)
	leftPQ := NewPriorityQueue(true)
	rightPQ := NewPriorityQueue(false)
	ansList := make([]int, 0)
	ans := 0
	for i := 0; i < M; i++ {
		rightPQ.Push(A[i])
	...
	for i := 0; i < N-M; i++ {
		...
		ans += insert
		if leftPQ.Size() > K {
			toRight, _ := leftPQ.Pop()
			ans -= toRight
			rightPQ.Push(toRight)
		}

		// delete
		maxLeft, _ := leftPQ.Pop()
		leftPQ.Push(maxLeft) // 一旦戻す
		if delete > maxLeft {
			rightPQ.Erase(delete)
		} else {
			leftPQ.Erase(delete)
			ans -= delete
			toLeft, _ := rightPQ.Pop()
			leftPQ.Push(toLeft)
			ans += toLeft
		}
		ansList = append(ansList, ans)
