...
func main() {
	...
	// value.keyは自身のカード
	// value.sizeは山の数
	// value.underCardは一つ下のカードのポインタ
	for i, p := range P {
		pi := Key(p)
		minKey, ok := avl.LowerBound(pi)
		if !ok {
			avl.Insert(pi, 1)
			continue
		}
		size, _ := avl.Get(minKey)
		under[p-1] = int(minKey)-1
		// out(fmt.Sprintf("%v on %v", p, minKey))
		// 食べられるとき
		if int(size)+1 == k {
			ate[p-1] = i+1
			for j:=under[p-1];j!=-1;j=under[j]{
				ate[j] = i+1
			}
			avl.Delete(minKey)
			continue
