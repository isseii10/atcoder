...
func main() {
	...
	for i:=0;i<q;i++ {
		...
		li := sort.Search(len(idxs[x]), func(i int) bool {return idxs[x][i] >= l})
		ri := sort.Search(len(idxs[x]), func(i int) bool {return idxs[x][i] > r})
		// // out(fmt.Sprintf("x = %v", x))
		// out(fmt.Sprintf("idxs[x] = %d", idxs[x]))
		// out(fmt.Sprintf("li = %v", li))
		// out(fmt.Sprintf("ri = %v", ri))
		out(ri-li)
